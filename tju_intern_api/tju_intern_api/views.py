...
from .youdao import *
# C:\Users\admin\Desktop\jupyter-workspace\shixun\project\服务器相关\tju_intern_api\tju_intern_api
cwd = os.getcwd()
dir = r'C:\Users\admin\Desktop\jupyter-workspace\shixun\project\服务器相关\tju_intern\index\static\userImage'
...
def GradeAduio(text,AUDIO_PATH):
    x_appid = '5ed7af6d'
    api_key = '9bf3b408f3aed077fbf6cab47b107d0e'
    ...
    result = eval(result)
    data = result['data']
    read_sentence = data['read_sentence']
    rec_paper = read_sentence['rec_paper']
    read_sentence = rec_paper['read_sentence']
    total_score = read_sentence['total_score']
...
def evaluateVoice(request):
    audioName = request.GET.get('audioName','')
    audio_file = (os.path.join( dir, audioName))
    ...
    text = ASR(audio_file)
    # text = eval(result['data'])[0]['onebest']
    score = GradeAduio(text ,(os.path.join( dir, audioName)))
    re = {'context':text,'score':score}
    return HttpResponse(json.dumps(re, ensure_ascii=False),content_type='application/json; charset=utf-8')

def audioEmotion(request):
    audioName = request.GET.get('audioName', '')
    audioE,textE,context = predictOnce((os.path.join( dir, audioName)))
    re = {'context': context, 'audioE': audioE.tolist(),'textE': textE.tolist()}
    return HttpResponse(json.dumps(re, ensure_ascii=False),content_type='application/json; charset=utf-8')

def getImgRes(request):
    imgName = request.GET.get('imgName', '')
    print(imgName)
    # imgpath=((os.path.join( cwd, 'tju_intern_api','dog.jpg')))
    imgpath=((os.path.join(dir, imgName)))
    caption = img2sent(imgpath)
    transRes = trans(caption)[0]
    text_res = getSentRes(transRes)
    re = {'context': caption, 'chinese': transRes, 'emotion': text_res.tolist()}
    print(re)
    print(text_res)
    return HttpResponse(json.dumps(re, ensure_ascii=False), content_type='application/json; charset=utf-8')
